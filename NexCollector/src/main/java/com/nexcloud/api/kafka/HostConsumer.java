 ...
 *
 */
public class HostConsumer 
{
	private static HostConsumer thisObj 	= null;
	static final Logger 	logger 		= LoggerFactory.getLogger(HostConsumer.class);
	
	private KafkaConsumer<String, String> consumer = null;
	
	public synchronized static HostConsumer getInstance(){
		if ( thisObj == null ){
			logger.debug("Kafka Consumer!!!!!!!!!!!!!!!");
			try {
				thisObj = new HostConsumer();
	...
	public boolean init( String kafka_zookeeper, String kafka_host, String kafka_port, String topic, String kafka_group )
	{
		...
		try{
			  logger.error("Host Consumer init 실행["+kafka_group+","+kafka_host+":"+kafka_port+","+kafka_zookeeper+"]");
		      ...
		      logger.error("Host Consumer init finish");
		      return true;
		}catch(Exception e){
			e.printStackTrace();
