 ...
 *
 */
public class LogDBConsumer 
{
	private static LogDBConsumer thisObj 	= null;
	static final Logger 	logger 		= LoggerFactory.getLogger(LogDBConsumer.class);
	
	private KafkaConsumer<String, String> consumer = null;
	
	public synchronized static LogDBConsumer getInstance(){
		if ( thisObj == null ){
			logger.debug("Kafka Consumer!!!!!!!!!!!!!!!");
			try {
				thisObj = new LogDBConsumer();
	 ...
	 */
	public synchronized boolean  init( String kafka_zookeeper, String kafka_host, String kafka_port, String topic, String kafka_group )
	{
		...
		try{
			  logger.error("Consumer init 실행!!!!!!!!!!!!!!!");
			  logger.error("Trace db kafka_group:::"+kafka_group);
	...
	public ConsumerRecords<String, String> read(  )
	{
		...
		try{
			//logger.error("Elastic Consumer read 실행!!!!!!!!!!!!!!!");
