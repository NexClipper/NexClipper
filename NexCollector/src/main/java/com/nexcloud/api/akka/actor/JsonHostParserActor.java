	...
	public void exec( SendData sendData, String data )
	{
		...
		try{
			...
			if( cpu.getCpu_user_per() < 100 )
			{
				...
				//////////////////
				msg_cluster += "host,cluster_id="+cluster_id+",host_name="+header.getNode_name()+",host_ip="+header.getNode_ip();
				
				// CPU
				msg_cluster +=" load1="+cpu.getLoad1()+",load5="+cpu.getLoad5()+",load15="+cpu.getLoad15()+",cpu_total="+cpu.getCpu_total()+",cpu_used_percent="+cpu.getCpu_per()+",cpu_idle_percent="+cpu.getCpu_idle_per()+",cpu_irq_percent="+cpu.getCpu_irq_per()+",cpu_nice_percent="+cpu.getCpu_nice_per()+",cpu_sorfirq_percent="+cpu.getCpu_sorfirq_per()+",cpu_stolen_percent="+cpu.getCpu_stolen_per()+",cpu_sys_percent="+cpu.getCpu_sys_per()+",cpu_user_percent="+cpu.getCpu_user_per()+",cpu_wait_percent="+cpu.getCpu_wait_per();
				
				// Memory
				msg_cluster +=",mem_total="+mem.getTotal()+",mem_free="+mem.getFree()+",mem_used="+mem.getUsed()+",mem_actual_free="+mem.getActual_free()+",mem_actual_used="+mem.getActual_used()+",mem_free_percent="+mem.getFree_per()+",mem_used_percent="+mem.getUsed_per();
				...
				if( swap.getFree_per() != null && swap.getUsed_per() != null)
				{
					...
					msg_cluster +=",swap_total="+swap.getTotal()+",swap_free="+swap.getFree()+",swap_pagein="+swap.getPageIn()+",swap_pageout="+swap.getPageOut()+",swap_used="+swap.getUsed()+",swap_free_percent="+swap.getFree_per()+",swap_used_percent="+swap.getUsed_per();
				}
				else
				{
					...
					msg_cluster +=",swap_total="+swap.getTotal()+",swap_free="+swap.getFree()+",swap_pagein="+swap.getPageIn()+",swap_pageout="+swap.getPageOut()+",swap_used="+swap.getUsed()+",swap_free_percent=0,swap_used_percent=0";
				}
				...
				msg_cluster += ",proc_total="+procStatus.getTotal()+",proc_sleeping="+procStatus.getSleeping()+",proc_running="+procStatus.getRunning()+",proc_stopped="+procStatus.getStopped()+",proc_zombie="+procStatus.getZombie()+",proc_idle="+procStatus.getIdle()+",timestamp="+resData.getTimestamp()+"\n";
			}
			
			// Process
			processThread.set(header.getNode_name(), header.getNode_ip(), cluster_id, processes, mem.getTotal(), resData.getTimestamp());
			
			// Network
			ifThread.set(header.getNode_name(), header.getNode_ip(), cluster_id, net.getIfaces(), resData.getTimestamp());
			
			// Disk
			diskThread.set(header.getNode_name(), header.getNode_ip(), cluster_id, disks, resData.getTimestamp());
			
			// Cpu
			cpuThread.set( header.getNode_name(), header.getNode_ip(), cluster_id, cpus, resData.getTimestamp());
			...
			redisCluster.put(cluster_id+"_"+Const.HOST, header.getNode_ip(), Util.beanToJson(host));
