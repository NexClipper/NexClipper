...
{
	private static SnapshotConsumer thisObj 	= null;
	static final Logger 	logger 		= LoggerFactory.getLogger(SnapshotConsumer.class);
	
	private KafkaConsumer<String, String> consumer = null;
	
	public synchronized static SnapshotConsumer getInstance(){
		if ( thisObj == null ){
			logger.debug("Kafka Consumer!!!!!!!!!!!!!!!");
			try {
				thisObj = new SnapshotConsumer();
	...
	public boolean init( String kafka_zookeeper, String kafka_host, String kafka_port, String topic, String kafka_group )
	{
		...
		try{
			  logger.error("Snapshot Consumer init 실행["+kafka_group+","+kafka_host+":"+kafka_port+","+kafka_zookeeper+"]");
