...
function mdtEvent() {
	$('#replicasetListTableArea').on('click', 'input:checkbox[type=checkbox]', function() {
...
function drawReplicaset (data) {
	...
	arr.forEach(function (item){
		var m = {
			...
			pod : item.status.replicas + "/" + item.status.fullyLabeledReplicas,
	...
	var columns = getColumns("replicaset", "check");
	new MDT().area("replicasetListTableArea").columns(columns).data(tableData).event(mdtEvent).makeSearch().draw();
...
new Namespace().area("namespaceArea").thisNs(namespace).thisWorkload("replicaset").draw();
</script>