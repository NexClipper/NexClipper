...
function getChartData( flag ) {
	type			= flag;
	new Client().url("/api/v1/incident/event/"+type+"?start_time="+START_TIME).callback( drawChart ).getNotCluster();
}
...
function doPagingClick( )
{
	...
	new Client().url("/api/v1/incident/event?"+param).callback(drawContainerList).getNotCluster();
}
...
$(function(){
	...
	new Client().url("/api/v1/incident/event?start_time="+START_TIME).callback(drawContainerList).getNotCluster();
