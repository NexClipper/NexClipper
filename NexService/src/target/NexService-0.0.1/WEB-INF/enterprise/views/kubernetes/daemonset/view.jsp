...
function mdtEvent() {
	$('#daemonsetArea').on('click', 'input:checkbox[type=checkbox]', function() {
...
function drawDaemonset (data) {
	...
	var columns = getColumns("daemonset", "check");
	new MDT().area("daemonsetArea").columns(columns).data(tableData).event(mdtEvent).makeSearch().draw();
...
new Namespace().area("namespaceArea").thisNs("${namespace}").thisWorkload("daemonset").draw();
