...
function drawCpuUsage (startTime, time) {
	...
	.callback(function(data){
		new BasicLine().area("cpuChartArea").setYAxisMax(100).colors(colors.cpuUsed).data(data, "used", "Cpu Usage").draw();
	}).get();
}
...
function drawDiskUsage (disks) {
	...
	new MDT().area("diskUsedListArea").columns(columns).data(tableData).makeSearch().draw();
}
function drawNetworkUsage (ifaces) {
	...
	new MDT().area("networkUsedListArea").columns(columns).data(tableData).makeSearch().draw();
}
