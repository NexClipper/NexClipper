...
function drawCpuTotal (data, bindData) {
	var basicLine = new BasicLine().area("cpuChartArea").colors(colors.cpuTotal).data(data, "cpuTotal", "CPU Total").draw();
	function drawCpuUsed(data) {
		basicLine.appendColors(colors.cpuUsed).appendData(data, "cpuUsed", "CPU Used");
	}
...
function drawMemoryTotal (data, bindData) {
	var basicLine = new BasicLine().area("memoryChartArea").colors(colors.cpuTotal).data(data, "memoryTotal", "Memory Total").draw();
	function drawMemoryUsed(data) {
		basicLine.appendColors(colors.memoryUsed).appendData(data, "memoryUsed", "Memory Used");
	}
...
function drawPodTotal (data, bindData) {
	var basicLine = new BasicLine().area("podChartArea").colors(colors.podTotal).data(data, "podTotal", "Pod Total").draw();
	function drawPodUsed(data) {
		basicLine.appendColors(colors.podUsed).appendData(data, "podUsed", "Pod Used");
	}
...
function drawDaemonsets (data) {
	...
	data.items.forEach(function(item){
		...
		listData.push({
			name : "<a href='/kubernetes/daemonset/"+item.metadata.name+"/detail?namespace=" + item.metadata.namespace + "'><span style='cursor:pointer;' class='font-weight-bold m--font-brand'>" + item.metadata.name + "</span></a>",
			status : status,
...
function drawDeployment (data) {
	...
	data.items.forEach(function(item){
		...
		listData.push({
			name : "<a href='/kubernetes/deployment/"+item.metadata.name+"/detail?namespace=" + item.metadata.namespace + "'><span style='cursor:pointer;' class='font-weight-bold m--font-brand'>" + item.metadata.name + "</span></a>",
			status : status,
			namespace : item.metadata.namespace,
