...
function mdtEvent() {
	$('#statefulsetListTableArea').on('click', 'input:checkbox[type=checkbox]', function() {
...
function drawStatefulset (data) {
	...
	var columns = getColumns("statefulset", "check");
	new MDT().area("statefulsetListTableArea").columns(columns).data(tableData).event(mdtEvent).makeSearch().draw();
...
new Namespace().area("namespaceArea").thisNs(namespace).thisWorkload("statefulset").draw();
</script>